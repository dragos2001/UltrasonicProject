...
def read_measurement(meas_poll):
    if meas_poll.poll(10):
        string = sys.stdin.readline()
        if string !='\n' :
            return string
    return None
        
print("Give name of the object:")
meas_obj = sys.stdin.readline()
...
while True:
    ...
    if specs!=None:
        print(f"iteration {meas_counter}{specs}")
        data = get_array()
        collect_data(data, meas_obj, specs, meas_counter)
        meas_counter+=1
        time.sleep(1)
